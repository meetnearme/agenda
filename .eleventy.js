...
module.exports = function (eleventyConfig) {
    ...
    eleventyConfig.addShortcode(
        'postGrid',
        function (featured = true, count = 3, excludeSlug = '') {
            const posts = this.ctx.collections.posts || [];

            let filteredPosts = posts;
            if (featured) {
                filteredPosts = posts.filter(
                    (post) => post.data.featuredpost === true
                );
            }
            if (excludeSlug) {
            ...
            let html =
                '<div class="mb-8"><div class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">';
    ...
    eleventyConfig.addPairedShortcode('verticalTilesGrid', function (content) {
        ...
        let html =
            '<div class="mb-8"><ol class="grid list-none gap-5 p-0 lg:grid-rows-3">';
    ...
    eleventyConfig.addShortcode(
        ...
        function (variant = 'card') {
            ...
            if (mode === 'iframe' && hasValidEmbed) {
                ...
                const embedWithoutScripts = embedCode.replace(
